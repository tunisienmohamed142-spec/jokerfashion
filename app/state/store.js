...
export function addCartItem(itemInput) {
  ...
  const regularPrice = Number(itemInput.priceSek) || 0;
  const salePriceSek = itemInput.salePriceSek ? Number(itemInput.salePriceSek) : null;
  const effectivePrice = salePriceSek && salePriceSek < regularPrice ? salePriceSek : regularPrice;
  const normalizedItem = {
    ...
    ...(salePriceSek && salePriceSek < regularPrice ? { compareAtPriceSek: regularPrice } : {}),
  };
